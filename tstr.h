...
#define sprintf_api(_buf, _size, _fmt, _v)          sprintf_s(_buf, _size, _fmt, _v) 
#define swprintf_api(_buf, _size, _fmt, _v)         swprintf_s(_buf, _size, _fmt, _v) 
...
#define sprintf_api(_buf, _size, _fmt, _v)          sprintf(_buf, _fmt, _v)
#define swprintf_api(_buf, _size, _fmt, _v)         swprintf(_buf, _fmt, _v)
...
#pragma region _operator
    ...
    inline str_base operator+(const_pointer s)
    {
        ...
        str_base ret(m_ptr->len + __lstrlen(s));
...
private:
    ...
    inline static int __debug(str_base* pstr, size_t bufferSize, const_pointer format, va_list list)
    {
        ...
        int r = __vsprintf(buffer, bufferSize, format, list);
        while (r == -1)
        {
            bufferSize *= 2;
            buf.resize(bufferSize);
            buffer = buf.data();
            r = __vsprintf(buffer, bufferSize, format, list);
        }
...
#pragma region _append_assign
    ...
    {
        if (_count == 0)return *this;
        if (strLen == 0) strLen = __lstrlen(_ptr);
...
#pragma region text_resize_format_cmp_toup_tolo
    ...
    {
        if (!m_ptr) return 0;
        m_ptr->len = __lstrlen(m_ptr->pData);
        return m_ptr->len;
    }
    ...
    inline int sscanf(const_pointer format, ...)
    {
        ...
        va_list aptr;
        va_start(aptr, format);
        int ret = __vsscanf(m_ptr->pData, format, aptr);
        va_end(aptr);
        return ret;
    }
    ...
    inline static int sscanf(const_pointer buffer, const_pointer format, ...)
    {
        va_list aptr;
        va_start(aptr, format);
        int ret = __vsscanf(buffer, format, aptr);
        va_end(aptr);
        return ret;
    }
    ...
    inline static int compare(const_pointer s1, const_pointer s2, const size_t _start, const size_t _cmplen, bool isCase)
    {
        ...
        if (s1)len1 = __lstrlen(s1);
        if (s2)len2 = __lstrlen(s2);
        ...
        if (isCase)    // 区分大小写
            return __strncmp(p1, p2, cmpLen);
        return __strnicmp(p1, p2, cmpLen);
    }
    // 文本比较, 相等返回0, 小于返回负数, 大于返回正数
    // _dst    = 被比较的文本
    // isCase    = 是否区分大小写, true = 区分, false = 不区分
...
#pragma endregion
    // 字符串是否为空, 对象未初始化, 字符串长度为0, 都返回真, 成员函数等效于 size == 0
    inline bool empty()const { return (m_ptr == 0) || (m_ptr->pData[0] == 0 || m_ptr->len == 0); }
...
public:
    ...
    inline bool starts_with(const_pointer x, bool isCase = true) const
    {
        ...
        size_t len = __lstrlen(x);
        if (len > m_ptr->len)return false;  // 传入的字符串比本对象字符串还长, 那就一定不匹配

        if (!isCase)
        {
            return __strnicmp(m_ptr->pData, x, len) == 0;
        }
        return __strncmp(m_ptr->pData, x, len) == 0;
    }
    ...
    inline static bool starts_with(const_pointer _string, const_pointer x, bool isCase = true)
    {
        ...
        const size_t strLen = __lstrlen(_string);
        const size_t len = __lstrlen(x);
        if (len > strLen)return false;  // 传入的字符串比本对象字符串还长, 那就一定不匹配

        if (!isCase)
        {
            return __strnicmp(_string, x, len) == 0;
        }
        return __strncmp(_string, x, len) == 0;
    }
    ...
    inline bool ends_with(const_pointer x, bool isCase = true) const
    {
        ...
        if (!isCase)
        {
            return __stricmp(_start, x) == 0;
        }
        return __strcmp(_start, x) == 0;
    }
    ...
    {
        if (!_string || !_string[0] || !x || !x[0])return false;
        size_t strLen = __lstrlen(_string);
        size_t len = __lstrlen(x);
        if (len > strLen)return false;  // 传入的字符串比本对象字符串还长, 那就一定不匹配
...
#pragma region _find_erase_replace_left_right_
    ...
    {
        if (empty())return npos;
        return findEx(m_ptr->pData, _substr, m_ptr->len, __lstrlen(_substr), pos, c);
    }
    // 寻找字符串,从一个字符串中寻找一个字符串,成功返回找到位置,失败返回str_base::npos
    // _substr = 寻找的文本
    ...
    {
        if (empty())return npos;
        return rfindEx(m_ptr->pData, _substr, __lstrlen(_substr), pos, isCase);
    }
    // 倒找字符串,从一个字符串中寻找一个字符串,成功返回找到位置,失败返回str_base::npos
    // _substr = 寻找的文本
    // _strlen = 寻找文本的长度, 为0则 _substr必须以\0结尾
    ...
    inline bool replace(const_pointer _string, const_pointer _substr, const size_t _off = 0, size_t count = size_t(-1), bool isCase = true)
    {
        ...
        return replaceEx(*this, m_ptr->pData, _string, _substr, __lstrlen(_string), 0, _off, count, isCase);
    }
    ...
    inline str_base& insert(const size_t _off, const_pointer _str, const size_t _len = 0, const size_t _count = 1)
    {
        ...
        size_t len = _len ? _len : __lstrlen(_str);
    ...
    str_base substr(const_pointer s1, const_pointer s2, const size_t _off = 0, bool b = false)const
    {
        ...
        size_t len1 = __lstrlen(s1);
        size_t len2 = __lstrlen(s2);
    ...
    str_base TrimAll(const value_type _ch = ' ') const
    {
        ...
        size_t len = __lstrlen(pStr);    // 这里是去掉了左边的指定字符
        size_t i;
        str_base tmp;
        tmp.resize(len);
        pointer p = tmp.data();
...
#pragma region __inside_static_private
    ...
    // 分割文本, 参数1必须自己申请缓冲区传过来, 参数2是用于分割的文本
    inline static char* __strtok(char* s, const char* const substr, char** next_token)              { return ::strtok_api(s, substr, next_token); }
    inline static wchar_t* __strtok(wchar_t* s, const wchar_t* const substr, wchar_t** next_token)  { return ::wcstok_api(s, substr, (wchar_t**)next_token); }
    ...
    // 子文本比较,区分大小写,小于返回 str_base::npos,大于返回1,等于返回0
    inline static int __strncmp(const char* s1, const char* s2, size_t count)               { return strncmp(s1, s2, count); }
    inline static int __strncmp(const wchar_t* s1, const wchar_t* s2, size_t count)         { return wcsncmp(s1, s2, count); }
    inline static int __strcmp(const char* s1, const char* s2)                              { return strcmp(s1, s2); }
    inline static int __strcmp(const wchar_t* s1, const wchar_t* s2)                        { return wcscmp(s1, s2); }
    inline static int __stricmp(const char* s1, const char* s2)                             { return stricmp_api(s1, s2); }
    inline static int __stricmp(const wchar_t* s1, const wchar_t* s2)                       { return wcsicmp_api(s1, s2); }
    inline static int __strnicmp(const char* s1, const char* s2, size_t count)              { return strnicmp_api(s1, s2, count); }
    inline static int __strnicmp(const wchar_t* s1, const wchar_t* s2, size_t count)        { return wcsnicmp_api(s1, s2, count); }
    inline static int __sprintf(pointer const _Buffer, size_t const _BufferCount, value_type const* const _Format, ...)
    {
        va_list aptr;
        va_start(aptr, _Format);
        int r = __vsprintf(_Buffer, _BufferCount, _Format, aptr);
        va_end(aptr);
    }

   
    // 寻找文本, 从头开始寻找另一个文本, 找到返回找到文本的起始地址,失败返回0
    inline static char*          __find(char* const s1, const char* const s2)               { return strstr(s1, s2); }
    inline static const char*    __find(const char* const s1, const char* const s2)         { return strstr(s1, s2); }
    inline static wchar_t*       __find(wchar_t* const s1, const wchar_t* const s2)         { return wcsstr(s1, s2); }
    inline static const wchar_t* __find(const wchar_t* const s1, const wchar_t* const s2)   { return wcsstr(s1, s2); }

    // 倒找文本, 从尾开始寻找另一个文本, 找到返回找到文本的起始地址,失败返回0
    inline static char*          __rfind(char* const s1, char const s2)                     { return strrchr(s1, s2); }
    inline static const char*    __rfind(const char* s1, char const s2)                     { return strrchr(s1, s2); }
    inline static wchar_t*       __rfind(wchar_t* const s1, wchar_t const s2)               { return wcsrchr(s1, s2); }
    inline static const wchar_t* __rfind(const wchar_t* const s1, wchar_t const s2)         { return wcsrchr(s1, s2); }
    ...
    inline static int __sscanf(const char* buffer, const char* format, ...) {
        int _Result;
        va_list _ArgList;
        __crt_va_start(_ArgList, format);

        _Result = vsscanf_api(buffer, format, _ArgList);
        __crt_va_end(_ArgList);
        return _Result;
    }

    inline static int __sscanf(const wchar_t* buffer, const wchar_t* format, ...) {
        int _Result;
        va_list _ArgList;
        __crt_va_start(_ArgList, format);

        _Result = vswscanf_api(buffer, format, _ArgList);

        __crt_va_end(_ArgList);
        return _Result;
    }

    inline static int __vsscanf(const char* buffer, const char* format, va_list _ArgList) {
        return vsscanf_api(buffer, format, _ArgList);
    }

    inline static int __vsscanf(const wchar_t* buffer, const wchar_t* format, va_list _ArgList) {
        return vswscanf_api(buffer, format, _ArgList);
    }
...
public:
    ...
    static size_t strtok(const_pointer str, strArr<value_type>* arr, const_pointer _substr, const size_t _count = size_t(-1))
    {
        ...
        pointer ptr = __strtok(p->str, _substr, &next_token);
        ...
        while (ptr)
        {
            ...
            ptr = __strtok(NULL, _substr, &next_token);
        }
        p->count = n;
        if (!arr)
        {
    ...
    {
        return findEx(_str, _substr, __lstrlen(_str), __lstrlen(_substr), pos, isCase);
    }
    // 寻找字符串,从一个字符串中寻找一个字符串,成功返回找到位置,失败返回 str_base::npos
    // _substr = 寻找的文本
    ...
    {
        if (!_str)return str_base::npos;
        const size_t nArgStrLen = __lstrlen(_str);
        const size_t nArgSubstrLen = __lstrlen(_substr);
        ...
        while (pStart <= pEnd)
        {
            bool bCmp;
            if (isCase)
                bCmp = __strncmp(pStart, pDst, nSubLen) == 0;
            else
                bCmp = __strnicmp(pStart, pDst, nSubLen) == 0;
            if (bCmp)
            {
                nStart = pStart - _str;
                return nStart;
    ...
    {
        return rfindEx(_str, _substr, __lstrlen(_substr), pos, c);
    }
    // 倒找字符串,从一个字符串中寻找一个字符串,成功返回找到位置,失败返回str_base::npos
    // _substr = 寻找的文本
    // _strlen = 寻找文本的长度, 为0则 _substr必须以\0结尾
    ...
    static size_t rfindEx(const_pointer _str, const_pointer _substr, const size_t _strlen, const size_t pos, bool c)
    {
        ...
        if (strLen == 0)strLen = __lstrlen(_substr);
        size_t len = __lstrlen(_str);
        ...
        while (pStart >= _str)    // 如果当前指向的位置大于字符串的起始位置,就循环判断
        {
            bool bCmp = 0;
            if (c) //区分大小写
                bCmp = __strncmp(pStart, _substr, nSubLen) == 0;    // 区分大小写
            else
                bCmp = __strnicmp(pStart, _substr, nSubLen) == 0;    // 不区分大小写
    ...
    inline static bool leftEx(str_base& ret, const_pointer str, size_t _strlen, const size_t _count)
    {
        ...
        _strlen = _strlen ? _strlen : __lstrlen(str);
    ...
    inline static str_base right(const_pointer str, const_pointer s, bool b = false, const size_t _off = size_t(-1))
    {
        ...
        size_t len1 = __lstrlen(str);
        size_t len2 = __lstrlen(s);
    ...
    inline static str_base replace(const_pointer _str, const_pointer _string, const_pointer _substr, const size_t _off = 0, size_t count = size_t(-1), bool c = true)
    {
        ...
        size_t len = 0;
        if (_substr)
            len = __lstrlen(_substr);
        str_base ret;
        replaceEx(ret, _str, _string, _substr, __lstrlen(_string), len, _off, count, c);
        ret.Addref();
        return ret;
    }
    ...
    static bool replaceEx(str_base& ret, const_pointer _str, const_pointer _string, const_pointer _substr, size_t _strlen, size_t _sublen, size_t _off, size_t count, bool isCase)
    {
        ...
        {
            bool bCmp;
            if (isCase) bCmp = __strncmp(pStart, _string, strLen) == 0;
            else        bCmp = __strnicmp(pStart, _string, strLen) == 0;
        ...
        {
            ret.append(pSrc, __lstrlen(pSrc));
        }
